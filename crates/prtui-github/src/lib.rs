...
fn rest_url(repo: &Repo, path: &str) -> String {
    match enterprise_host(repo) {
...
fn web_url(repo: &Repo) -> String {
    ...
    format!("https://{host}/{}/{}", repo.namespace, repo.name)
}

fn graphql_url(repo: &Repo) -> String {
    match enterprise_host(repo) {
...
impl WirePullRequest {
    fn into_item(self, repo: Arc<Repo>) -> PullRequestListItem {
        PullRequestListItem {
            target: prtui_core::PullRequestTarget {
                repo,
                number: self.number,
            },
            title: self.title,
...
async fn reply(
    ...
    let in_reply_to = in_reply_to
        .parse::<u64>()
        .context("GitHub returned an invalid reply target")?;
    let token = write_token(repo).await?;
    let url = rest_url(
        repo,
        &format!(
...
async fn fetch_outage(repo: &Repo) -> Option<String> {
    ...
    if enterprise_host(repo).is_some() {
        return None;
    }
...
impl Provider for GitHub {
    ...
    fn pull_request_url(self, repo: &Repo, number: u32) -> String {
        format!("{}/pull/{number}", web_url(repo))
    }
...
mod tests {
    ...
    fn github_is_usable_through_the_provider_boundary() {
        ...
        assert_eq!(
            ...
            "https://github.com/owner/repo/blob/abc123/src/a%20b.rs#L7-L9"
        );
    }
    ...
    fn routes_enterprise_hosts_to_their_own_api_mount() {
        ...
        assert_eq!(
            rest_url(&public, "/repos/owner/repo/pulls"),
            "https://api.github.com/repos/owner/repo/pulls"
        );
        assert_eq!(graphql_url(&public), "https://api.github.com/graphql");

        // An explicit github.com is the public API, not an enterprise mount.
        let explicit = parse_repo("github.com/owner/repo").unwrap();
        assert_eq!(graphql_url(&explicit), "https://api.github.com/graphql");

        let enterprise = parse_repo("ghe.corp/owner/repo").unwrap();
        assert_eq!(
            rest_url(&enterprise, "/repos/owner/repo/pulls"),
