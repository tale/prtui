...
pub(super) fn file_page(bytes: &[u8]) -> Result<Vec<ChangedFile>> {
...
pub(super) fn files(bytes: &[u8]) -> Result<Vec<ChangedFile>> {
...
impl From<WireDiscussionComment> for Comment {
    fn from(comment: WireDiscussionComment) -> Self {
        Self {
            id: comment.id.into(),
            rest_id: comment.full_database_id.and_then(RestId::value),
...
pub(super) fn meta(value: Value) -> Result<Meta> {
    let response =
        serde_json::from_value(value).context("unexpected graphql response")?;
    into_meta(response)
}

pub(super) fn meta_bytes(bytes: &[u8]) -> Result<Meta> {
    let response =
...
pub(super) fn added_thread(value: Value) -> Result<AddedThread> {
...
pub(super) fn thread_variables(thread: NewThread) -> Value {
